...
class TDL():
    ...
    def learn(self, total_timesteps, learning_rate=0.0005, gamma=0.99, buffer_size=50000, exploration_initial_eps=1.0, 
        ...
        episode_last_estimated_value = np.array([]) # Array to store model estimation of episode value from last state
        ...
        while i_timestep < total_timesteps:
            ...
            episode_last_estimated_value = np.append(episode_last_estimated_value, estimate_value)
            # Plot training progress
            plot_save = i_episode % checkpoint_freq == 0 and i_episode > 1
            self._plot_callback(loss, episode_rewards, episode_initial_estimated_value, episode_last_estimated_value, max_tiles, epsilons, plot_save, checkpoint_path)
    ...
    def _plot_callback(self, loss, episode_rewards, episode_initial_estimated_value, episode_last_estimated_value, max_tiles, epsilons, plot_save, save_path):
        """
        ...
        - episode_last_estimated_value (list): A list of last estimated values per episode.
        ...
        axes[3].plot(episode_last_estimated_value, label="Episode Last Estimated Value")
        if len(episode_rewards) >= 100:
